...
// Définir les types pour les utilisateurs et les tâches
type Task = {
  id: string;
  title: string;
  description: string;
  dueDate?: string;
  status: "To Do" | "In Progress" | "Done"; // Statut de progression
};

type User = {
  id: string;
  name: string;
  tasks: Task[]; // Liste de tâches associées à cet utilisateur
};

export default function Dashboard() {
  const [users, setUsers] = useState<User[]>([]); // Liste des utilisateurs avec leurs tâches
  const [searchQuery, setSearchQuery] = useState(""); // Recherche
  const [filteredUsers, setFilteredUsers] = useState<User[]>([]); // Utilisateurs filtrés

  // Fonction pour récupérer les utilisateurs et leurs tâches
  const fetchUsersWithTasks = async () => {
    try {
      const response = await axiosClient.get("/tasks"); // Endpoint du backend
      if (Array.isArray(response.data)) {
        setUsers(response.data); // On suppose que le backend renvoie des utilisateurs avec des tâches
      } else {
        console.error("Invalid response format:", response.data);
      }
    } catch (error) {
  ...
  // Charger les utilisateurs et leurs tâches au montage
  useEffect(() => {
    fetchUsersWithTasks();
  }, []);

  // Filtrer les utilisateurs et leurs tâches à chaque changement de recherche
  useEffect(() => {
    if (searchQuery === "") {
      setFilteredUsers(users); // Si la recherche est vide, on montre tout
    } else {
