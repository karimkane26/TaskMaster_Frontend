...
const SignUpScreen = () => {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      toast.success("Inscription réussie ! Bienvenue sur TaskMaster.");

      // Rediriger vers l'écran de connexion
      router.push("/dashboard");
    } catch (error) {
      // Vérification et affichage des erreurs de l'API
      if (error.response) {
        const { data, status } = error.response;

        if (status === 400) {
          toast.error(data.message || "Données invalides. Veuillez vérifier les informations saisies.");
        } else if (status === 409) {
        ...
        toast.error("Impossible de joindre le serveur. Veuillez vérifier votre connexion internet.");
      } else {
        toast.error("Une erreur inconnue s'est produite. Veuillez réessayer.");
      }
    }
  };
